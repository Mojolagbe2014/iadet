...
if(!isset($_SESSION['IADETLoggedInAdmin']) || !isset($_SESSION["IADETadminEmail"])){ 
    ...
    if(filter_input(INPUT_POST, "fetchTutors") != NULL){
        ...
        $columns = array( 0 =>'id', 1 => 'name', 2 => 'qualification', 3 => 'field', 4 => 'bio', 5 => 'email', 6 => 'username', 7 => 'website',  8 => 'picture', 9 => 'visible');
    ...
    if(filter_input(INPUT_POST, "deleteThisTutor")!=NULL){
        ...
        if(count($errorArr) < 1 && $tutorImage != "")   {
            $pictureDelParam = true;
            if(file_exists(MEDIA_FILES_PATH."tutor/".$tutorImage)){
    ...
    if(filter_input(INPUT_POST, "updateThisTutor") != NULL){
        $postVars = array('id', 'name','qualification','field','bio','email','userName','website','picture');  // Form fields names

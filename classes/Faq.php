...
class Faq implements ContentManipulator{
    ...
    public function fetchForJQDT($draw, $totalData, $totalFiltered, $customSql="", $column="*", $condition="", $sort="id"){
        ...
        if(count($data)>0){
            foreach($data as $r){ 
                $result[] = array($r['id'], utf8_encode($r['question']), utf8_encode($r['answer']), utf8_encode($r['date_added']), utf8_encode(' <button data-date-added="'.$r['date_added'].'" data-id="'.$r['id'].'" class="btn btn-info btn-small edit-faq"  title="Edit"><i class="btn-icon-only icon-pencil"> </i> <span id="JQDTquestionholder" class="hidden">'.$r['question'].'</span> <span id="JQDTanswerholder" class="hidden">'.$r['answer'].'</span> </button> <button data-id="'.$r['id'].'" class="btn btn-danger btn-small delete-faq" title="Delete"><i class="btn-icon-only icon-trash"> </i> <span id="JQDTquestionholder2" class="hidden">'.$r['question'].'</span></button>'));
            }

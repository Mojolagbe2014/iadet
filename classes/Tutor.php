...
class Tutor implements ContentManipulator{
     ...
     */
    public function add(){
        $sql = "INSERT INTO tutor (name, qualification, field, bio, email, username, password, website, picture, visible) "
                ."VALUES ('{$this->name}','{$this->qualification}','{$this->field}','{$this->bio}','{$this->email}','{$this->userName}','".md5($this->passWord)."','{$this->website}','{$this->picture}','{$this->visible}')";
    ...
    public function delete(){
        $sql = "DELETE FROM tutor WHERE id = $this->id ";
    ...
    public function fetch($column="*", $condition="", $sort="id"){
        $sql = "SELECT $column FROM tutor ORDER BY $sort";
        if(!empty($condition)){$sql = "SELECT $column FROM tutor WHERE $condition ORDER BY $sort";}
        $data = $this->dbObj->fetchAssoc($sql);
        $result =array(); 
        if(count($data)>0){
            foreach($data as $r){
                $result[] = array("id" => $r['id'], "name" =>  utf8_encode($r['name']), 'qualification' =>  utf8_encode($r['qualification']), "field" =>  utf8_encode($r['field']), "bio" =>  utf8_encode(stripslashes(strip_tags($r['bio']))), "email" =>  utf8_encode($r['email']), "userName" =>  utf8_encode($r['username']), "website" =>  utf8_encode($r['website']), "picture" =>  utf8_encode($r['picture']), "visible" =>  utf8_encode($r['visible']));
            }
    ...
    public function update() {
        $sql = "UPDATE tutor SET name = '{$this->name}', qualification = '{$this->qualification}', field = '{$this->field}', bio = '{$this->bio}', email = '{$this->email}', username = '{$this->userName}', website = '{$this->website}', picture = '{$this->picture}' WHERE id = $this->id ";
        if(!empty($this->id)){
            $result = $this->dbObj->query($sql);

    ...
    public function fetchForJQDT($draw, $totalData, $totalFiltered, $customSql="", $column="*", $condition="", $sort="id"){
        ...
        if(count($data)>0){
            foreach($data as $r){ 
                ...
                $result[] = array($r['id'], utf8_encode(' <button data-id="'.$r['id'].'" data-image="'.$r['image'].'" data-name="'.$r['name'].'"  data-location="'.$r['location'].'" class="btn btn-danger btn-small delete-event" title="Delete"><i class="btn-icon-only icon-trash"> </i></button> <button  data-id="'.$r['id'].'"  data-image="'.$r['image'].'" data-name="'.$r['name'].'" data-location="'.$r['location'].'"  data-date-time="'.$r['date_time'].'" class="btn btn-info btn-small edit-event"  title="Edit"><i class="btn-icon-only icon-pencil"> </i> <span class="hidden" id="JQDTdescriptionholder">'.utf8_encode($r['description']).'</span> </button> <button data-id="'.$r['id'].'" data-name="'.$r['name'].'" data-status="'.$r['status'].'"  class="btn '.$fetEventRolCol.' btn-small activate-event"  title="'.$fetEventRolTit.'"><i class="btn-icon-only '.$fetEventStat.'"> </i></button>'), utf8_encode($r['name']), utf8_encode('<img src="../media/event/'.utf8_encode($r['image']).'" width="40" height="30" alt="Pix">'), StringManipulator::trimStringToFullWord(90, strip_tags(utf8_encode($r['description']))), utf8_encode($r['date_time']), utf8_encode($r['location']), utf8_encode($r['date_added']));//
            }

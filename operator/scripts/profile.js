$(document).ready(function(){
    ...
    var formVar = {id:sessionStorage.IADETadminId, name:sessionStorage.IADETAdminFullName, email:sessionStorage.IADETadminEmail, userName:sessionStorage.IADETAdminName, role:sessionStorage.IADETAdminRole};
    ...
    $("form#UpdateProfile").submit(function(e){ 
        ...
        $.ajax({
            ...
            success : function(data, status) {
                if(data.status === 1) { 
                    disableUpdateForm();
                    $("#messageBox").html('<div class="alert alert-'+alertType[data.status]+'"><button type="button" class="close" data-dismiss="alert">&times;</button>'+data.msg+'. Please wait ... </div>');
                    setInterval(function(){ logout(); }, 2500);
                }
                else if(data.status === 2 || data.status === 3 || data.status === 0 || data.status === 4) { 
                    $("#messageBox").html('<div class="alert alert-'+alertType[data.status]+'"><button type="button" class="close" data-dismiss="alert">&times;</button>'+data.msg+'. </div>');
                }
                else $("#messageBox").html('<div class="alert alert-info"><button type="button" class="close" data-dismiss="alert">&times;</button>'+data+'</div>');
            }
        });
        return false;
    });
    ...
    $("form#changeAdminPassword").submit(function(e){ 
        ...
        $.ajax({
            ...
            success : function(data, status) {
                if(data.status === 1) {  
                    $(".messageBox").html('<div class="alert alert-'+alertType[data.status]+'"><button type="button" class="close" data-dismiss="alert">&times;</button>'+data.msg+' Please wait ... Login out..</div>');
                    setInterval(function(){ logout(); }, 2500);
                }
                else if(data.status === 2 || data.status === 3 || data.status === 0 || data.status === 4) { 
                    $(".messageBox").html('<div class="alert alert-'+alertType[data.status]+'"><button type="button" class="close" data-dismiss="alert">&times;</button>'+data.msg+'. </div>');

$(document).ready(function(){
    $('form#ManageWebsiteFaq input, textarea').attr('disabled', 'disabled');
    $('form#ManageWebsiteFaq #updateContent').attr('disabled', 'disabled');
    $('#cancelEdit').text('Edit Details');
    
    $.ajax({
        url: "../REST/manage-website-faq.php",
        type: 'POST',
        cache: false,
        data: {fetchWebsiteFaq: 'true'},
        success : function(data, status) {
            if(data.status === 0 || data.status === 2 || data.status === 3 ){ 
                ...
                $.each(data.info, function(i, item) { 
                    formVar = {id:item.id, title:item.title, description:item.description, keywords:item.keywords};
    ...
    $('#cancelEdit').click(function(){ 
        if($(this).text() == 'Edit Details'){
            $(this).text('Cancel');
            $('form#ManageWebsiteFaq #updateContent').removeProp('disabled', 'disabled');
            $('form#ManageWebsiteFaq input, textarea').removeProp('disabled');
        }
        else{
    ...
    $("form#ManageWebsiteFaq").submit(function(e){ 
        e.stopPropagation(); 
        e.preventDefault();

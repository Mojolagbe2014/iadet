$(document).ready(function(){
    $('form#ManageWebsiteContact input, textarea').attr('disabled', 'disabled');
    $('form#ManageWebsiteContact #updateContent').attr('disabled', 'disabled');
    $('#cancelEdit').text('Edit Details');
    
    $.ajax({
        url: "../REST/manage-website-contact.php",
        type: 'POST',
        cache: false,
        data: {fetchWebsiteContact: 'true'},
        success : function(data, status) {
            if(data.status === 0 || data.status === 2 || data.status === 3 ){ 
                ...
                $.each(data.info, function(i, item) { 
                    formVar = {id:item.id, title:item.title, description:item.description, keywords:item.keywords, phone:item.phone, email:item.email, address:item.address};
    ...
    $('#cancelEdit').click(function(){ 
        if($(this).text() == 'Edit Details'){
            $(this).text('Cancel');
            $('form#ManageWebsiteContact #updateContent').removeProp('disabled', 'disabled');
            $('form#ManageWebsiteContact input, textarea').removeProp('disabled');  
        }
        else{
    ...
    $("form#ManageWebsiteContact").submit(function(e){ 
        e.stopPropagation(); 
        e.preventDefault();

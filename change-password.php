...
foreach ($userObj->fetchRaw("*", " confirmed = 1 AND id = $thisUserId ") as $user) {
    $userData = array('id', 'firstName', 'lastName','email','description','picture', 'phone', 'address', 'userName', 'dateRegistered', 'passWord');
...
$thisPage->title = StringManipulator::trimStringToFullWord(62, stripslashes(strip_tags("Change Password - International Academy of Dental Education and Training (IADET)")));
$thisPage->description = StringManipulator::trimStringToFullWord(150, stripslashes(strip_tags("Change profile password.")));
$thisPage->keywords = "profile, password, change";
...
<body class="page page-id-107 page-template-default custom-background woocommerce-account woocommerce-page tribe-theme-Guru page-template-page-php no-js">
        ...
        <div id="wrapper">
            ...
            <section class="breadcrumb-wrapper">
                    <div class="container">
                        ...
                        <a href="profile">Profile</a><span class="default" >  </span><h4>Change Password</h4>
                        </div>				  
                    </div>                      
            </section>
                        ...
                        <section id="main-course" class="course-container">
                            <article class="course post post-2541 type-course status-publish has-post-thumbnail hentry course-category-food">
                                <div class="woocommerce">
                                    ...
                                    <form action="REST/change-user-password.php" method="post" class="login" id="updateUserPassword" name="updateUserPassword">
                                        <p class="form-row form-row-wide">
                                            <label for="oldPassword">Current Password <span class="required">*</span></label>
                                            <input type="password" class="input-text" name="oldPassword" id="oldPassword" value="" />
                                        </p>
                                        <p class="form-row form-row-wide">
                                            <label for="newPassword">New Password <span class="required">*</span></label>
                                            <input class="input-text" type="password" name="newPassword" id="newPassword" />
                                        </p>
                                        <p class="form-row form-row-wide">
                                            <label for="confirmPassword">Confirm Password <span class="required">*</span></label>
                                            <input class="input-text" type="password" name="confirmPassword" id="confirmPassword" />
                                        </p>
                                        <p class="form-row">
                                            <input type="hidden" id="LoggedInUserId" name="LoggedInUserId" value="<?php echo $userObj->id; ?>" />
                                            <input type="submit" class="button" name="change-password" value="Update Password" /> 
                                            <br/>
                                        </p>
                                    </form>
                                </div>
    ...
    <script>
        ...
        jQuery(document).ready(function($){
            $('form#updateUserPassword').submit(function (e) {
                e.stopPropagation(); 
                e.preventDefault(); 
                var formDatas = $(this).serialize();
                var passUpdFormRest = $(this).attr("action");
                ajaxQuery(passUpdFormRest, "POST", formDatas, "Password Successfully Changed !!!", "Password Update Failed !!!", 'Reloading ...', "ERROR: ");
                return false;
            });

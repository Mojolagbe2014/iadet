...
$thisPage->title = StringManipulator::trimStringToFullWord(62, stripslashes(strip_tags('All Upcoming Events - '.WEBSITE_AUTHOR)));
$thisPage->description = StringManipulator::trimStringToFullWord(150, stripslashes(strip_tags('All upcoming seminars, conferences, trainings and events by '.WEBSITE_AUTHOR)));
$thisPage->keywords = 'seminars, training, conference, event, upcoming, all';
...
<head>
    ...
    <link rel='stylesheet' id='jeweltheme-jquery-ui-style-css'  href='plugins/wp-awesome-event/jquery-uif9b8.css?ver=4.0' type='text/css' media='all' />
    ...
    <style type='text/css'>n.tp-caption a{color:#ff7302;text-shadow:none;-webkit-transition:all 0.2s ease-out;-moz-transition:all 0.2s ease-out;-o-transition:all 0.2s ease-out;-ms-transition:all 0.2s ease-out}.tp-caption a:hover{color:#ffa902}</style>
    ...
    <script type='text/javascript' src='plugins/wp-awesome-event/accordion68b3.js?ver=1'></script>
...
<body class="archive post-type-archive post-type-archive-event custom-background tribe-theme-Guru page-template-archive-php no-js">
        ...
        <?php include('includes/header.php'); ?>        
            ...
            <section class="breadcrumb-wrapper">
                ...
                <div class="breadcrumb">
                    <a href="index">Home</a><span class="default" >  </span><h4>Events</h4></div>              
                </div>                      
            </section>
                ...
                <div class="container">
                    <section class="content-full-width" id="primary">
                        <div class="column dt-sc-one-column blog-fullwidth">
                            <?php 
                                $eventCounter = 1;
                                foreach($eventObj->fetchRaw("*", " status = 1 AND date_time >= NOW() ")as $thisEvent) { 
                                    $dateTime = explode(' ', $thisEvent['date_time']);
                                    
                                    $dateParam = explode('/', $dateTime[0]);
                                    $dateObj   = DateTime::createFromFormat('!m', $dateParam[1]);
                            ?>
                            <article id="post-537" class="post-537 event type-event status-publish hentry blog-post">
                                <div class="post-details">                
                                    <div class="date"><p><span><?php echo $dateParam[2]; ?></span><br /><?php echo $dateObj->format('F'); ?><br /><?php echo $dateParam[0]; ?></p></div>                                  
                                    <div class="post-comments">
                                        <span><i class="fa fa-clock-o"></i> <?php echo $dateTime[1]; ?></span>                                 
                                    </div>                          
                                </div>
                                <div class="post-content">
                                    <div class="entry-detail" style="text-align:justify">
                                        <h2><?php echo $eventCounter.". ".$thisEvent['name']; ?></h2>
                                        <p><img src="media/event/<?php echo $thisEvent['image']; ?>" style="width:180px;height:120px;float:left;margin-right:10px;"/><?php echo $thisEvent['description']; ?></p>
                                        <div class="social-bookmark">
                                            <p>
                                            <span><i class="fa fa-map-marker"></i> <?php echo $thisEvent['location']; ?></span> <br/>
                                            <span><i class="fa fa-calendar"></i> <?php echo $dateParam[2].' '.$dateObj->format('F').', '.$dateParam[0]; ?></span> <br/>
                                            <span><i class="fa fa-clock-o"></i> <?php echo $dateTime[1]; ?></span> <br/>
                                            </p>
                                        </div>                              
                                    </div>
                                    <br/><br/>

...
$thisPage->title = StringManipulator::trimStringToFullWord(62, stripslashes(strip_tags("My Manually Logged Courses - International Academy of Dental Education and Training (IADET)")));
$thisPage->description = StringManipulator::trimStringToFullWord(150, stripslashes(strip_tags($userObj->userName ."'s manually logged CPDs.")));
$thisPage->keywords = $userObj->userName.", cpd, cpds, course";
    ...
    #activity td{
     ...
     #footerExport td{
       ...
       text-align:center;
        ...
        @media screen and (max-width: 800px) { button#exportAsPdf { font-size:11px;margin:5px;padding:5px;}}
    </style>
	...
	<!-- wrapper div starts here -->
        <div id="wrapper">
            ...
            <section class="breadcrumb-wrapper">
                    <div class="container">
                        <h1><?php echo $userObj->userName; ?>'s Logged CPDs</h1>
                        <div class="breadcrumb">
                        <a href="index">Home</a><span class="default" >  </span> <a href="profile">Profile</a><span class="default" >  </span><h4>Logged CPDs</h4>
                        </div>				  
                    </div>                      
            </section>
                        ...
                        <section id="main-course" class="course-container">
                            <article class="course post post-2541 type-course status-publish has-post-thumbnail hentry course-category-food">
                                <div class="module-head">
                                    <table>
                                        <tr id="footerExport">
                                            <td id="exportexcel"><img src="tabletopdf/img/xls.png" title="Export to Excel" /></td>
                                            <td id="exportpdf"><img src="tabletopdf/img/pdf.png" title="Export to PDF" /></td>
                                            <td id="exportimage"><img src="tabletopdf/img/png.png" title="Export to PNG" /></td>
                                            <td id="exportcsv"><img src="tabletopdf/img/csv.png" title="Export to CSV" /></td>
                                            <td id="exportword"><img src="tabletopdf/img/word.png" title="Export to Word" /></td>
                                            <td id="exporttxt"><img src="tabletopdf/img/txt.png" title="Export to TXT" /></td>
                                            <td id="exportxml"><img src="tabletopdf/img/xml.png" title="Export to XML" /></td>
                                            <td id="exportppt"><img src="tabletopdf/img/ppt.png" title="Export to PPT" /></td>
                                        </tr>
                                    </table>
<!--                                    <button type="button" id="exportAsPdf" style="background:darkred;margin-bottom:20px;">Export PDF</button>-->
                                </div>
                                
                                <div class="module-body table">
                                    
                                    <table id="activity" cellpadding="0" cellspacing="0" border="0" class="display"  width="100%">
                                        <thead>
                                            <tr>
                                                <th>Course Speciality</th>
                                                <th>Topic</th>
                                                <th>CPD/CME Points/Hours</th>
                                                <th>Location</th>
                                                <th>Date</th>
                                                <th>Comment</th>
                                                <th>Certificate</th>
                                        ...
                                        <tbody>
                                        <?php 
                                        $userCourse = new UserCourse($dbObj); $itemNo = 0; $certificateNo = 0; $cpdPoint = 0; 
                                        foreach ($userCourse->fetchRaw("*", " user = $userObj->id ") as $loggedCpd) { 
                                            if($loggedCpd['point'] != ''){  $cpdPoint += $loggedCpd['point']; }
                                            if($loggedCpd['certificate'] !='') { $certificateNo++;}
                                        ?>
                                            <tr>
                                                <td><?php echo $loggedCpd['speciality']; ?></td>
                                                <td><a href='logged-cpd?id=<?php echo $loggedCpd['id']; ?>' class='tag-link-64' title='<?php echo $loggedCpd['topic']; ?>'><?php echo $loggedCpd['topic']; ?></a></td>
                                                <td><?php echo $loggedCpd['point']; ?></td>
                                                <td><?php echo $loggedCpd['location']; ?></td>
                                                <td><?php echo $loggedCpd['attendance_date']; ?></td>
                                                <td><?php echo StringManipulator::trimStringToFullWord(62, $loggedCpd['comment']); ?></td>
                                                <td><?php if($loggedCpd['certificate'] !='') { ?><a href="media/user-course-certificate/<?php echo $loggedCpd['certificate']; ?>">Download</a> <?php } ?></td>
                                            </tr>
                                        <?php $itemNo++; } ?>
                                            <tr>
                                                <td> <strong>TOTAL</strong> </td><td> <?php echo ucwords(Number::convertToWords($itemNo)).' Courses Logged'; ?> </td><td> <?php echo $cpdPoint.' CPD/CME Points/Hours Obtained '; ?> </td><td> ---- </td><td> ---- </td><td> ---- </td><td> <?php echo ucwords(Number::convertToWords($certificateNo)).' Certificates Obtained'; ?> </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
    ...
    <script src="operator/scripts/jquery-1.9.1.min.js" type="text/javascript"></script>
    <script src="tabletopdf/tableExport.js" type="text/javascript"></script>
    <script src="tabletopdf/jquery.base64.js" type="text/javascript"></script>
    <script src="tabletopdf/html2canvas.js" type="text/javascript"></script>
    <script src="tabletopdf/jspdf/libs/base64.js" type="text/javascript"></script>
    <script src="tabletopdf/jspdf/libs/sprintf.js" type="text/javascript"></script>
    <script src="tabletopdf/jspdf/jspdf.js" type="text/javascript"></script>
    <script>
        $(document).ready(function () {
            ...
            $('#exportpdf').bind('click', function (e) {
                $('#activity').tableExport({ type: 'pdf', escape: 'false', htmlContent:'false', pdfFontSize:'8', ignoreColumn: [] });
            });
